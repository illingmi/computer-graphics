...
// I added a comment, does this still work?
...
minFilter = THREE.CubeRefractionMapping;
...
var toonMaterial = new THREE.ShaderMaterial( {
        uniforms: {
           lightPosition: {value: new THREE.Vector3(0.0,0.0,-1.0) },
           myColor: { value: new THREE.Vector4(0.5,1.0,0.8,1.0) }
        },
	vertexShader: document.getElementById( 'myVertShader' ).textContent,
...
sphereB = new THREE.Mesh( new THREE.SphereGeometry(2, 20, 10), toonMaterial);
...
function checkKeyboard() {
  ...
  lightSphere.position.set(light.position.x, light.position.y, light.position.z);
    ...
    if (sphereA.position.y <= -1.0){
        //sphereA = new THREE.Mesh( new THREE.SphereGeometry(2, 20, 10), toonMaterial);
        //sphereA = sphereB;
        //int ax = sphereA.position.x;
        //float ay = sphereA.position.y;
        //float az = sphereA.position.z;
        //sphereB.position.set = (sphereA.position.x, sphereA.position.y, sphereA.position.z);
        
        scene.add(sphereB);
        sphereB.position.set = (sphereA.position.x, sphereA.position.y, sphereA.position.z);
        //sphereB.position.set(x, y, z);
